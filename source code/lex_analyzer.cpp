...
bool LexAnalyzer::Open_Success(const string code_filename) {
...
void LexAnalyzer::GetIndex(WordInfo& getword) {

	if (getword.type == LEOF)
		getword.index = -1;
	else if (getword.type == LINTEGER)
		getword.index = 3;
	else if (getword.type == LFLOAT)
		getword.index = 4;
	else if (getword.type == LKEYWORD || getword.type == LTYPE) {
		if (getword.value == "if" || getword.value == "else" || getword.value == "while" || getword.value == "return" || getword.value == "break" || getword.value == "for")
			getword.index = 1;
		else if (getword.value == "using")
			getword.index = -4;
		else if (getword.value == "namespace")
			getword.index = -5;
		else if (getword.value == "int" || getword.value == "void" || getword.value == "double" || getword.value == "float" || getword.value == "bool" || getword.value == "char" || getword.value == "define")
			getword.index = 2;
		else
			getword.index = 20000;
	}
		...
		getword.index = 0;
	else if (getword.type == LDELIMITER) {
		if (getword.value == "(")
			getword.index = 18;
		else if (getword.value == ")")
			getword.index = 19;
		else if (getword.value == "[")
			getword.index = 20;
		else if (getword.value == "]")
	...
	else if (getword.type == LUNKNOWN)
		getword.index = -10;
	else if (getword.type == LOPERATOR) {
		if (getword.value == "+")
			getword.index = 6;
		else if (getword.value == "-")
			getword.index = 7;
	...
	else if (getword.type == LINCLUDE)
		getword.index = -2;
	else if (getword.type == LDEFINE)
		getword.index = -3;
	else
		getword.index = 30000;
}
...
WordInfo LexAnalyzer::GetWord() {
	...
	if (getword.type == LEOF)
		...
		getword.type == LDELIMITER || getword.type == LOPERATOR)
...
void LexAnalyzer::PrintInfo(WordInfo word) {
	/*if (word.type == LEOF)
		return;*/
	if (step_count == 1) {
		cout << "Step:  " << "<Type,Index,Value>" << endl;
		lex_analyze_result << "Step:  " << "< Type , Index , Value >" << endl;
	}
	cout << "No:" << setiosflags(ios::left) << setw(3) << step_count << "<" << setw(10) << OutputType[word.type] << ", " << setw(2) << word.index << ", " << word.value << " >";
	lex_analyze_result << "No:" << setiosflags(ios::left) << setw(3) << step_count << "< " << setw(10) << OutputType[word.type] << ", " << setw(2) << word.index << ", " << word.value << " >";
	cout << endl;
...
LexAnalyzer::LexAnalyzer(string resultfile) {
	line_count = 1;
	step_count = 1;
	lex_analyze_result.open(resultfile, ios::out);
}
